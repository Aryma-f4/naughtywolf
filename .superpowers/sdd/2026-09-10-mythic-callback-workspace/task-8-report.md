...
- RED/GREEN: added a processing-transfer cancellation regression and a real implant `nw/upload` slot cancellation regression. `nw/killtask` now removes the active transfer slot only after recording cancellation, keeps the task registered for the complete I/O/publication lifetime, emits one terminal cancelled result, and repository result handling atomically marks the transfer cancelled so the next FIFO task advances.
- Stable storage: Unix constructor walks and creates every evidence path component through `openat`/`mkdirat` with `O_NOFOLLOW`, retains the root descriptor, enumerates cleanup with `fdopendir`/`readdir`, and deletes only through `unlinkat`; expired cleanup no longer uses pathname-based metadata/removal. Windows source coverage adds `CreateFileW` handles with `FILE_FLAG_OPEN_REPARSE_POINT`/`FILE_FLAG_BACKUP_SEMANTICS`, ancestor/final reparse-attribute checks, and `CreateHardLinkW` no-replace publication for TransferStore and implant sidecars. Existing destination/reparse failures leave the destination untouched.
- Lease safety: writes validate/renew the SQLite file lease before and after each write; long verification hashes renew at bounded intervals and validate again before publication. `active_file_lease_blocks_takeover_until_safe_expiry` proves a second store cannot write while an unexpired owner holds the lease and can resume only after expiry.
- Production integration: `file_backed_application_transfer_survives_http_disconnect_and_db_reopen` uses a temporary file-backed SQLite database, real `Repository`/`TransferStore`, authenticated application routes over a real loopback TCP listener, distinct 3 KiB upload/download payloads, server abort/recreation, DB pool close/reopen and store reconstruction, durable offset checks, SHA-256 checks, provisional-to-terminal publication confirmation, and FIFO follower advancement. It passed 1/1.
- Focused evidence: `cargo test -p naughtywolf --test callback_workspace_test active_file_lease_blocks_takeover_until_safe_expiry -- --nocapture` passed 1/1; processing cancellation passed 1/1; file-backed application integration passed 1/1; `cargo test -p nw-implant runtime::tests::killtask_cancels_a_real_upload_slot_and_emits_terminal_result -- --nocapture` passed 1/1.
- Specified evidence: `node --test tests/callback_workspace_test.cjs` passed 22/22; `cargo test -p nw-profile` passed 26/26; implant download filter passed 1 unit + 1 E2E and upload filter passed 3 (including forced 502 reconnect); `cargo fmt --all -- --check && git diff --check` passed; fresh `cargo test --workspace` passed all suites (40 naughtywolf library, 3 binary, 42 callback, 13 audit, 4 auth, 16 check-runner, 6 config, 7 policy, 61 portal, 21 repository, 4 console, 40 implant library, 12 implant C2 E2E, 17 callback-controls, 6 modules, 26 profile, 30 server; all doc-tests passed).
- Windows limitation: `cargo check -p nw-implant --target x86_64-pc-windows-gnu` was attempted, but this host's target installation lacks the `core` standard library (`E0463: can't find crate for core`). The Windows source APIs/types are present behind `cfg(windows)` and the dependency features are declared; no Windows runtime claim is made.
